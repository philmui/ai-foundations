...
- **data/dirty_dataset.csv**: Intentionally messy dataset (80 rows) with:
  - 5 exact duplicates
  ...
  - Mixed date formats
- **lab_the_curator.py**: Educational cleaning pipeline script
- **slides.html**: Interactive presentation (keyboard navigation)
- **data/output/**: Directory for clean Train.csv and Test.csv (generated by lab)
...
# Or with uv
uv pip install python-dotenv pandas
```

## Running the Lab

```bash
python lab_the_curator.py
```
...
8. Export Train.csv and Test.csv to data/output/
